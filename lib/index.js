...
function binaryFilesPath(binaryFileName) {
  return`${__dirname.replace('app.asar', 'app.asar.unpacked')}/bin/${binaryFileName}`
}

function getProcessListMac() {
  let macBinaryName = process.arch === 'arm64'
  ? 'getProcessListDarwinArm'
  : 'getProcessListDarwinIntel'

  return getProcessListBin(binaryFilesPath(macBinaryName))
}
...
async function getProcessList() {
  const processListMap = {
    win32: getProcessListBin(binaryFilesPath('getProcessListWin32.exe')),
    darwin: getProcessListMac(),
    linux: getLinuxProc,
  };
