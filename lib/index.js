...
async function killProcByPID(pid) {
  ...
  try {
    switch (plat) {
      case 'win32': // NOTE: This can all be simplified to return winAPI.forceKillProcByPID if ungraceful killing is okay, the entire purpose of said code is to exit process gracefully
        winAPI.killProcByPID(tempPid);
        await new Promise((r) => setTimeout(r, 5000)); // Wait for 5 seconds for the program to close
